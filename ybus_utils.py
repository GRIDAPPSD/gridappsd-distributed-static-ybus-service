...
handler = logging.FileHandler('DistributedYBusUtils.log',mode='w',encoding='utf-8')
...
def perLengthPhaseImpedanceLineConfigs(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
        ...
        if configIsValid:
            for phaseImpedanceData in perLengthImpedance.PhaseImpedanceData: # type: ignore
                ...
                if len(nullAttributes) > 0:
                    logger.debug(f"perLengthPhaseImpedanceLineConfigs():PerLengthImpedance:{perLengthImpedance.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                    nullAttributes.clear()
                if len(desiredInfo) > 0 and not dictContainsNone:
                    rv.append(copy.deepcopy(desiredInfo))
                desiredInfo.clear()
    logger.debug(f'perLengthPhaseImpdenaceLineConfigs returns: {json.dumps(rv,indent=4,sort_keys=True)}')
    return rv


def perLengthPhaseImpedanceLineNames(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
            ...
            for acLineSegmentPhase in line.ACLineSegmentPhases: # type: ignore
                ...
                for i,v in desiredInfo.items():
                    if v["value"] is None:
                        ...
                        nullAttributes.append(i)
                if len(nullAttributes) > 0:
                    logger.debug(f"perLengthPhaseImpedanceLineNames():ACLineSegment:{line.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                    nullAttributes.clear()
                if len(desiredInfo) > 0 and not dictContainsNone:
                    rv.append(copy.deepcopy(desiredInfo))
                desiredInfo.clear()
    logger.debug(f'perLengthPhaseImpedanceLineNames returns: {json.dumps(rv,indent=4,sort_keys=True)}')
    return rv


def perLengthSequenceImpedanceLineConfigs(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
        ...
        if sequenceImpedanceIsValid: # type: ignore
            ...
            for i,v in desiredInfo.items():
                if v["value"] is None:
                    ...
                    nullAttributes.append(i)
            if len(nullAttributes) > 0:
                logger.debug(f"perLengthSequenceImpedanceLineConfigs():perLenghtSequenceImpedance:{sequenceImpedance.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                nullAttributes.clear()
            if len(desiredInfo) > 0 and not dictContainsNone:
                rv.append(copy.deepcopy(desiredInfo))
            desiredInfo.clear()
    logger.debug(f'perLengthSequenceImpedanceLineConfigs returns: {json.dumps(rv,indent=4,sort_keys=True)}')
    return rv


def perLengthSequenceImpedanceLineNames(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
        ...
        if isinstance(line.PerLengthImpedance,cim.PerLengthSequenceImpedance): # type: ignore
            ...
            for i,v in desiredInfo.items():
                if v["value"] is None:
                    ...
                    nullAttributes.append(i)
            if len(nullAttributes) > 0:
                logger.debug(f"perLengthSequenceImpedanceLineNames():ACLineSegment:{line.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                nullAttributes.clear()
            if len(desiredInfo) > 0 and not dictContainsNone:
                rv.append(copy.deepcopy(desiredInfo))
            desiredInfo.clear()
    logger.debug(f'perLengthSequenceImpedanceLineNames returns: {json.dumps(rv,indent=4,sort_keys=True)}')
    return rv


def acLineSegmentLineNames(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
            ...
            for p in line.WireSpacingInfo.WirePositions: # type: ignore
                ...
                for i,v in desiredInfo.items():
                    if v["value"] is None:
                        ...
                        nullAttributes.append(i)
                if len(nullAttributes) > 0:
                    logger.debug(f"wireInfoSpacing():ACLineSegment:{line.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                    nullAttributes.clear()
                if len(desiredInfo) > 0 and not dictContainsNone:
                    rv.append(copy.deepcopy(desiredInfo))
                desiredInfo.clear()
...
def wireInfoConcentricNeutral(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
    ...
    for line in acLineSegments.values():
        for acLineSegmentPhase in line.ACLineSegmentPhases: # type: ignore
            if isinstance(acLineSegmentPhase.WireInfo,cim.ConcentricNeutralCableInfo):
                ...
                desiredInfo["diameter_jacket"] = {"value":concentricNeutralWireInfo.diameterJacket}
                ...
                for i,v in desiredInfo.items():
                    if v["value"] is None:
                        ...
                        nullAttributes.append(i)
                if len(nullAttributes) > 0:
                    logger.debug(f"wireInfoConcentricNeutral():ACLineSegment:{line.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                    nullAttributes.clear()
                if len(desiredInfo) > 0 and not dictContainsNone:
                    rv.append(copy.deepcopy(desiredInfo))
                desiredInfo.clear()
...
def wireInfoTapeShield(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
            ...
            if isinstance(acLineSegmentPhase.WireInfo,cim.TapeShieldCableInfo):
                ...
                for i,v in desiredInfo.items():
                    if v["value"] is None:
                        ...
                        nullAttributes.append(i)
                if len(nullAttributes) > 0:
                    logger.debug(f"wireInfoTapeShield():ACLineSegment:{line.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                    nullAttributes.clear()
                if len(desiredInfo) > 0 and not dictContainsNone:
                    rv.append(copy.deepcopy(desiredInfo))
                desiredInfo.clear()
...
def wireInfoLineNames(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
...
def powerTransformerEndXfmrImpedances(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
                    ...
                    if toTransformerEnd.mRID in transformerEnds: # type: ignore
                        ...
                        for i,v in desiredInfo.items():
                            if v["value"] is None:
                                ...
                                nullAttributes.append(i)
                        if len(nullAttributes) > 0:
                            logger.debug(f"powerTransformerEndXfmrImpedances():PowerTransformer:{powerTransformer.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                            nullAttributes.clear()
                        if len(desiredInfo) > 0 and not dictContainsNone:
                            rv.append(copy.deepcopy(desiredInfo))
                        desiredInfo.clear()
    logger.debug(f'powerTransformerEndXfmrImpedances returns: {json.dumps(rv,indent=4,sort_keys=True)}')
    return rv


def powerTransformerEndXfmrNames(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
    ...
    for powerTransformer in powerTransformers.values():
        for powerTransformerEnd in powerTransformer.PowerTransformerEnd: # type: ignore
            ...
            if isinstance(powerTransformerEnd.Terminal, cim.Terminal) and isinstance(powerTransformerEnd.Terminal.ConnectivityNode, cim.ConnectivityNode):
            ...
            for i,v in desiredInfo.items():
                if v["value"] is None:
                    ...
                    nullAttributes.append(i)
            if len(nullAttributes) > 0:
                logger.debug(f"powerTransformerEndXfmrNames():PowerTransformer:{powerTransformer.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                nullAttributes.clear()
            if len(desiredInfo) > 0 and not dictContainsNone:
                rv.append(copy.deepcopy(desiredInfo))
            desiredInfo.clear()
...
def transformerTankXfmrRated(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
        ...
        for transformerEndInfo in transformerTankInfo.TransformerEndInfos:
            ...
            for i,v in desiredInfo.items():
                if v["value"] is None:
                    ...
                    nullAttributes.append(i)
            if len(nullAttributes) > 0:
                logger.debug(f"transformerTankXfmrRated():TransformerTank:{transformerTank.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                nullAttributes.clear()
            if len(desiredInfo) > 0 and not dictContainsNone:
                rv.append(copy.deepcopy(desiredInfo))
            desiredInfo.clear()
...
def transformerTankXfmrSct(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
                ...
                for groundedEnd in shortCircuitTest.GroundedEnds:
                    ...
                    for i,v in desiredInfo.items():
                        if v["value"] is None:
                            ...
                            nullAttributes.append(i)
                    if len(nullAttributes) > 0:
                        logger.debug(f"transformerTankXfmrSct():TransformerTank:{transformerTank.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                        nullAttributes.clear()
                    if len(desiredInfo) > 0 and not dictContainsNone:
...
def transformerTankXfmrNames(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
        ...
        for transformerTankEnd in transformerTank.TransformerTankEnds: # type: ignore
            ...
            for i,v in desiredInfo.items():
                if v["value"] is None:
                    ...
                    nullAttributes.append(i)
            if len(nullAttributes) > 0:
                logger.debug(f"transformerTankXfmrNames():TransformerTank:{transformerTank.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                nullAttributes.clear()
            if len(desiredInfo) > 0 and not dictContainsNone:
                rv.append(copy.deepcopy(desiredInfo))
            desiredInfo.clear()
...
def switchingEquipmentSwitchNames(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
    ...
    switchEquipment = {**loadBreakSwitches,**reclosers,**breakers,**fuses,**sectionalisers,**jumpers,**disconnectors,**groundDisconnectors}
    desiredInfo= {}
            ...
            for switchPhase in switchEq.SwitchPhase: # type: ignore
                ...
                for i,v in desiredInfo.items():
                    if v["value"] is None:
                        ...
                        nullAttributes.append(i)
                if len(nullAttributes) > 0:
                    logger.debug(f"switchingEquipmentSwitchNames():Switch:{switchEq.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                    nullAttributes.clear()
                if len(desiredInfo) > 0 and not dictContainsNone:
                    rv.append(copy.deepcopy(desiredInfo))
                desiredInfo.clear()
    ...
    logger.debug(f'switchingEquipmentSwitchNames returns: {json.dumps(rv,indent=4,sort_keys=True)}')
    return rv


def shuntElementCapNames(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
            ...
            for shuntCompensatorPhase in linearShuntCompensator.ShuntCompensatorPhase: # type: ignore
                ...
                for i,v in desiredInfo.items():
                    if v["value"] is None:
                        ...
                        nullAttributes.append(i)
                if len(nullAttributes) > 0:
                    logger.debug(f"shuntElementCapNames():Capacitor:{linearShuntCompensator.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                    nullAttributes.clear()
                if len(desiredInfo) > 0 and not dictContainsNone:
                    rv.append(copy.deepcopy(desiredInfo))
                desiredInfo.clear()
...
def transformerTankXfmrNlt(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
            ...
            for noLoadTest in transformerEndInfo.EnergisedEndNoLoadTests:
                ...
                for i,v in desiredInfo.items():
                    if v["value"] is None:
                        ...
                        nullAttributes.append(i)
                if len(nullAttributes) > 0:
                    logger.debug(f"transformerTankXfmrNlt():TransformerTank:{transformerTank.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                    nullAttributes.clear()
                if len(desiredInfo) > 0 and not dictContainsNone:
                    rv.append(copy.deepcopy(desiredInfo))
                desiredInfo.clear()
...
def powerTransformerEndXfmrAdmittances(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> List[Dict]:
    rv = []
    if isinstance(distributedArea, DistributedModel):
        distributedAreaID = distributedArea.feeder.mRID
    else:
            ...
            if powerTransformerEnd.CoreAdmittance is not None:
                ...
                for i,v in desiredInfo.items():
                    if v["value"] is None:
                        ...
                        nullAttributes.append(i)
                if len(nullAttributes) > 0:
                    logger.debug(f"powerTransformerEndXfmrAdmittances():PowerTransformer:{powerTransformer.name} contained the following Null attributes:\n{json.dumps(nullAttributes, indent=4, sort_keys=True)}")
                    nullAttributes.clear()
                if len(desiredInfo) > 0 and not dictContainsNone:
                    rv.append(copy.deepcopy(desiredInfo))
                desiredInfo.clear()
    logger.debug(f'powerTransformerEndXfmrAdmittances returns: {json.dumps(rv,indent=4,sort_keys=True)}')
    return rv
...
def fillYbusPerLengthPhaseImpedanceLines(distributedArea: DistributedModel | SwitchArea | SecondaryArea, Ybus: Dict):
...
def fillYbusPerLengthSequenceImpedanceLines(distributedArea: DistributedModel | SwitchArea | SecondaryArea, Ybus: Dict):
...
def fillYbusACLineSegmentLines(distributedArea: DistributedModel | SwitchArea | SecondaryArea, Ybus: Dict):
...
def diagZprim(wireinfo: str, wire_cn_ts, neutralFlag, R25, GMR, CN_strand_count, CN_strand_rdc, CN_strand_gmr, CN_strand_radius, CN_diameter_jacket, TS_tape_thickness, TS_diameter_screen):
...
def fillYbusWireInfoAndWireSpacingInfoLines(distributedArea: DistributedModel | SwitchArea | SecondaryArea, Ybus: Dict):
...
def fillYbusPowerTransformerEndXfmrs(distributedArea: DistributedModel | SwitchArea | SecondaryArea, Ybus: Dict):
...
def fillYbusTransformerTankXfmrs(distributedArea: DistributedModel | SwitchArea | SecondaryArea, Ybus: Dict):
...
def fillYbusSwitchingEquipmentSwitches(distributedArea: DistributedModel | SwitchArea | SecondaryArea, Ybus: Dict):
...
def fillYbusShuntElementShunts(distributedArea: DistributedModel | SwitchArea | SecondaryArea, Ybus: Dict):
...
def calculateYbus(distributedArea: DistributedModel | SwitchArea | SecondaryArea) -> Dict:
    ...
    logger.debug(f"feederYbusService Ybus after fillYbusPerLengthPhaseImpedanceLines is:\n{json.dumps(Ybus, indent=4, sort_keys=True, cls=ComplexEncoder)}")
    fillYbusPerLengthSequenceImpedanceLines(distributedArea, Ybus)
    logger.debug(f"feederYbusService Ybus after fillYbusPerLengthSequenceImpedanceLines is:\n{json.dumps(Ybus, indent=4, sort_keys=True, cls=ComplexEncoder)}")
    fillYbusACLineSegmentLines(distributedArea, Ybus)
    logger.debug(f"feederYbusService Ybus after fillYbusACLineSegmentLines is:\n{json.dumps(Ybus, indent=4, sort_keys=True, cls=ComplexEncoder)}")
    fillYbusWireInfoAndWireSpacingInfoLines(distributedArea, Ybus)
    logger.debug(f"feederYbusService Ybus after fillYbusWireInfoAndWireSpacingInfoLines is:\n{json.dumps(Ybus, indent=4, sort_keys=True, cls=ComplexEncoder)}")
    line_count = countUniqueYbus(Ybus)
    logger.debug('\nLine_model # entries: ' + str(line_count))
    fillYbusPowerTransformerEndXfmrs(distributedArea, Ybus)
    logger.debug(f"feederYbusService Ybus after fillYbusPowerTransformerEndXfmrs is:\n{json.dumps(Ybus, indent=4, sort_keys=True, cls=ComplexEncoder)}")
    fillYbusTransformerTankXfmrs(distributedArea, Ybus)
    logger.debug(f"feederYbusService Ybus after fillYbusTransformerTankXfmrs is:\n{json.dumps(Ybus, indent=4, sort_keys=True, cls=ComplexEncoder)}")
    ...
    logger.debug(f"feederYbusService Ybus after fillYbusSwitchingEquipmentSwitches is:\n{json.dumps(Ybus, indent=4, sort_keys=True, cls=ComplexEncoder)}")
    count = countUniqueYbus(Ybus)
    ...
    logger.debug(f"feederYbusService Ybus after fillYbusShuntElementShunts is:\n{json.dumps(Ybus, indent=4, sort_keys=True, cls=ComplexEncoder)}")
